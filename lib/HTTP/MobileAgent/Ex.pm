...
# Extension for HTTP::MobileAgent: on Jun. 29th, 2004 's data
...
*{"HTTP::MobileAgent::Vodafone\::is_station"} = sub 
  {
    ...
    return 0 if ($version > 4.2); 	# VGS is not support Station
...
{
  require HTTP::MobileAgent::Ex::AirHPhoneData;
  my $self = shift;
  my ($model) = split(/,/,$self->model);
  my $display = $HTTP::MobileAgent::Ex::AirHPhoneData::DisplayMap->{$model};
  ...
  *{"HTTP::MobileAgent::EZweb\::serial_number"} = 
  ...
  if ($ez_realnameAsModel)
  {
    *{"HTTP::MobileAgent::EZweb\::model"} = sub
    {
      require HTTP::MobileAgent::Ex::EZwebData;
      my $self = shift;
      return $HTTP::MobileAgent::Ex::EZwebData::Device2model->{$self->device_id};
    };
  }
  ...
  if ($ah_operaAsMobile)
  {
    $HTTP::MobileAgent::MobileAgentRE = HTTP::MobileAgent::Ex::maRE();
  }
}
